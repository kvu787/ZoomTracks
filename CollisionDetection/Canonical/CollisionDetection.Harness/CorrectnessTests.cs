...
namespace CollisionDetection.Harness
{
    internal static class CorrectnessTests
    {
        ...
        private static void TestPublicValueContracts()
        {
            ...
            RectanglePose defaultPose = default(RectanglePose);
            ...
            CoordinateXY defaultCoordinate = default(CoordinateXY);
        ...
        private static void TestDetectorConstructionContracts()
        {
            ...
            foreach (DetectorKind kind in DetectorKinds())
            {
                ...
                List<CoordinateXY> invalidSecond = new List<CoordinateXY>
                {
                    new CoordinateXY(0, 0),
                    new CoordinateXY(1, 0),
                };
                ...
                ICollisionDetector detector = Create(
                    ...
                    valid.CloneOutline2());
                AssertThrows<ArgumentException>(
                    () => detector.IsColliding(default(RectangleLocalBounds), default(RectanglePose)),
                    kind + " rejects default bounds");
            }

            List<CoordinateXY> subnormalTriangle = new List<CoordinateXY>
            {
        ...
        private static void TestAlgorithmSettingsAndFallbacks()
        {
            ...
            UniformGridCollisionDetector overflowGrid = new UniformGridCollisionDetector(
            ...
            QueryInput[] queries =
            {
                new QueryInput(Bounds(-1, -1, 1, 1), Pose(0, 0, 0)),
                new QueryInput(Bounds(-1, -1, 1, 1), Pose(10, 0, 0)),
                new QueryInput(Bounds(-20, -20, 20, 20), Pose(0, 0, 0)),
            };
            ...
            BvhCollisionDetector singleEdgeLeaves = new BvhCollisionDetector(
        ...
        {
            List<CoordinateXY> points = new List<CoordinateXY>();
            for (int y = -2; y <= 2; ++y)
            {
                for (int x = -2; x <= 2; ++x)
                {
        ...
        {
            Random random = new Random(0x51a9c3);
            ...
            for (int i = 0; i < 20_000; ++i)
            {
                ...
                CoordinateXY a = new CoordinateXY(originX, originY);
                CoordinateXY b = new CoordinateXY(originX + dx, originY + dy);
                CoordinateXY c = new CoordinateXY(originX + dx * 2.0f, originY + dy * 2.0f);
                ...
                CoordinateXY perturbed = new CoordinateXY(c.X, perturbedY);
        ...
        {
            Random random = new Random(0x42c011);
            ...
            foreach (int outerCount in sizes)
            {
                ...
                for (int i = 0; i < queryCount; ++i)
                {
                    ...
                    RectangleLocalBounds bounds = new RectangleLocalBounds(
                        ...
                        6.5f,
                        4.25f);
                    RectanglePose pose = new RectanglePose(
            ...
            List<CoordinateXY> outline2)
        {
            switch (kind)
            {
                case DetectorKind.Linear:
                    return new LinearScanCollisionDetector(outline1, outline2);
                case DetectorKind.Bvh:
                    return new BvhCollisionDetector(outline1, outline2);
                case DetectorKind.Grid:
                    return new UniformGridCollisionDetector(outline1, outline2);
                default:
