...
# # Monitor 1, dx11, 2560*1440, exclusive fullscreen (fullscreen optimizations must be disabled in process properties)
# $process = `
#     Start-Process `
#         -FilePath "C:\Users\k\Repository\Unity\ZoomTracks\ZoomTracks\MyBuildOutput\ZoomTracks.exe" `
#         -ArgumentList "-monitor 1 -force-d3d11 -force-gfx-direct -window-mode exclusive -screen-width 2560 -screen-height 1440 -logFile `"$($unityLogFilePath)`" -timestamps -refreshRate -1 -stutterLogFilePath `"$($stutterLogFilePath)`"" `
#         -PassThru
