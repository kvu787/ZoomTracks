"""Experimental TrackBuilder using offset-aware adaptive sampling for vanilla curves.

The public API is ``build_track(W, H, segment_length, material_names)``.
Mesh outlines retain the production behavior. Smooth curve outlines are sampled
from a high-resolution Blender evaluation. The track-facing edge remains exactly
on the authored evaluated outline while the away-facing offset is simplified
adaptively before output is committed transactionally.
"""
...
def _contact_and_offset_reference(
