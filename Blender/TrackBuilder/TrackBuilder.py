...
def _synthetic_example(number: int) -> tuple[float, float, float, list[str]]:
    ...
    if number == 2:
        ...
        raise ValueError(f"Unknown synthetic example {number}")
...
def generate_examples(output_directory: str, sample_path: str) -> list[str]:
    """Generate the ten example files required by the design specification."""

    output_directory = os.path.abspath(output_directory)
    sample_path = os.path.abspath(sample_path)
    if not os.path.isfile(sample_path):
        raise TrackBuilderValidationError(f"Sample input does not exist: {sample_path}")
    os.makedirs(output_directory, exist_ok=True)

    filenames = [
        "TrackBuilderExample01_SampleInput.blend",
        "TrackBuilderExample02_NoInner.blend",
        "TrackBuilderExample03_OneInner.blend",
        "TrackBuilderExample04_MultipleInner.blend",
        "TrackBuilderExample05_Concave.blend",
        "TrackBuilderExample06_Transformed.blend",
        "TrackBuilderExample07_Curves.blend",
        "TrackBuilderExample08_ReversedWinding.blend",
        "TrackBuilderExample09_Collinear.blend",
        "TrackBuilderExample10_DenseLongTarget.blend",
    ]
    written: list[str] = []

    bpy.ops.wm.open_mainfile(filepath=sample_path)
    sample_parameters = (0.3, 0.8, 2.5, ["red", "blue"])
    build_track(*sample_parameters)
    _record_parameters(*sample_parameters)
    first_path = os.path.join(output_directory, filenames[0])
    bpy.ops.wm.save_as_mainfile(filepath=first_path, check_existing=False)
    written.append(first_path)

    for number in range(2, 11):
        parameters = _synthetic_example(number)
        build_track(*parameters)
        _record_parameters(*parameters)
        path = os.path.join(output_directory, filenames[number - 1])
...
def _main() -> None:
    ...
    mode.add_argument("--build", action="store_true", help="Build the current file")
    mode.add_argument(
        "--generate-examples",
        action="store_true",
        help="Generate all ten design examples",
    )
    ...
    if arguments.generate_examples:
        paths = generate_examples(arguments.examples_dir, arguments.sample)
        for path in paths:
