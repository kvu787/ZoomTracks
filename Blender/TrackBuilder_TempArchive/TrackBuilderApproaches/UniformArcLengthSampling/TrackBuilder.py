"""Experimental TrackBuilder using uniform arc-length sampling for vanilla curves.

The public API is ``build_track(W, H, segment_length, material_names)``.
Mesh outlines retain the production behavior. Smooth source and offset paths are
independently resampled at uniform world-space intervals, while the source path
is constrained to the authored evaluated outline, before output is committed.
"""
...
def _curve_reference_resolution(obj: bpy.types.Object) -> int:
    curve = obj.data
    spline = curve.splines[0]
    base = max(int(curve.resolution_u), int(spline.resolution_u))
    return min(
        CURVE_MAXIMUM_RESOLUTION,
        max(CURVE_REFERENCE_MINIMUM_RESOLUTION, base * CURVE_REFERENCE_RESOLUTION_MULTIPLIER),
    )
...
def _sample_closed_polyline(
    ...
    if edge_length <= 0.0:
        return points[edge_index].copy(), edge_index, 0.0
    parameter = (distance - cumulative[edge_index]) / edge_length
    return (
        points[edge_index].lerp(points[(edge_index + 1) % len(points)], parameter),
        edge_index,
        parameter,
    )


def _uniform_source_distances(
    cumulative: list[float],
    count: int,
    epsilon: float,
) -> list[float]:
    """Return uniform distances plus every authored vertex distance."""

    perimeter = cumulative[-1]
    candidates = sorted(
        [index * perimeter / count for index in range(count)]
        + cumulative[:-1]
    )
    distances: list[float] = []
    for distance in candidates:
        if not distances or distance - distances[-1] > epsilon:
            distances.append(distance)
    return distances


def _align_dense_reference(
    authored_source: list[Vector],
    dense_source: list[Vector],
    dense_offset: list[Vector],
) -> tuple[list[Vector], list[Vector]]:
    """Rotate dense source/offset stations to the authored canonical start."""
    ...
    return (
        dense_source[start:] + dense_source[:start],
        dense_offset[start:] + dense_offset[:start],
    )


def _uniform_curve_outline(
    outline: _Outline,
    width: float,
    ...
    source_cumulative = _closed_cumulative_lengths(outline.points)
    source_perimeter = source_cumulative[-1]
    requested_source_count = max(3, math.ceil(source_perimeter / UNIFORM_ARC_LENGTH))
    source_distances = _uniform_source_distances(
        source_cumulative,
        requested_source_count,
        epsilon,
    )
    if len(source_distances) > CURVE_MAXIMUM_EVALUATED_POINTS:
        raise TrackBuilderGeometryError(
            f"Curve {outline.object_name!r} requires {len(source_distances)} uniform source samples; "
            f"the experimental maximum is {CURVE_MAXIMUM_EVALUATED_POINTS}"
        )
    points = [
    ...
    offset_points = None
    offset_fractions = None
    if offset_left is not None:
        dense_offset = _stable_curve_offset_points(dense_source, width, offset_left)
        dense_source, dense_offset = _align_dense_reference(
            outline.points,
            dense_source,
            dense_offset,
        )
        offset_cumulative = _closed_cumulative_lengths(dense_offset)
        offset_perimeter = offset_cumulative[-1]
        offset_count = max(3, math.ceil(offset_perimeter / UNIFORM_ARC_LENGTH))
        if offset_count > CURVE_MAXIMUM_EVALUATED_POINTS:
            raise TrackBuilderGeometryError(
                f"Curve {outline.object_name!r} requires {offset_count} uniform offset samples; "
                f"the experimental maximum is {CURVE_MAXIMUM_EVALUATED_POINTS}"
            )
        offset_points = []
        offset_fractions = []
        for index in range(offset_count):
            distance = index * offset_perimeter / offset_count
            point, edge_index, parameter = _sample_closed_polyline(
                dense_offset,
                offset_cumulative,
                distance,
            )
            source_position = (edge_index + parameter) * len(outline.points) / len(dense_offset)
            source_edge = min(int(math.floor(source_position)), len(outline.points) - 1)
            source_parameter = source_position - source_edge
            source_distance = source_cumulative[source_edge] + source_parameter * (
                source_cumulative[source_edge + 1] - source_cumulative[source_edge]
            )
            offset_points.append(point)
            offset_fractions.append(source_distance / source_perimeter)
    return _Outline(
        outline.object_name,
        outline.material,
        points,
        ...
        sampling_method=(
            f"uniform_contact=authored,arc_length={UNIFORM_ARC_LENGTH:g},"
...
def _refine_classified_outlines(
    ...
    refined_ground = (
        _uniform_curve_outline(ground, width, None, epsilon) if ground.is_curve else ground
    )
    refined_outer = (
        _uniform_curve_outline(outer, width, False, epsilon) if outer.is_curve else outer
    )
    refined_inner = [
        _uniform_curve_outline(outline, width, True, epsilon) if outline.is_curve else outline
