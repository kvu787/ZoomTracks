...
class TrackBuilderTests(unittest.TestCase):
    ...
    def test_resolution_issue_example_is_uniform_and_contacts_source_exactly(self) -> None:
        ...
        self.assertLess(len(outer.points), 3000)
        self.assertLess(len(inner[0].points), 3000)
        for outline in [outer, *inner]:
            ...
            maximum_source_edge = max(
                (outline.points[(index + 1) % len(outline.points)] - point).length
                for index, point in enumerate(outline.points)
            )
            maximum_offset_edge = max(
                (outline.offset_points[(index + 1) % len(outline.offset_points)] - point).length
                for index, point in enumerate(outline.offset_points)
            )
            self.assertLessEqual(maximum_source_edge, TrackBuilder.UNIFORM_ARC_LENGTH * 1.001)
            self.assertLessEqual(maximum_offset_edge, TrackBuilder.UNIFORM_ARC_LENGTH * 1.001)
